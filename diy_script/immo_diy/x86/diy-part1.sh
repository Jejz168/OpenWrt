...
if [ -f "bin/targets/x86/64/immortalwrt-x86-64-generic-squashfs-combined.img.gz" ]; then
    mv bin/targets/x86/64/immortalwrt-x86-64-generic-squashfs-combined.img.gz \
       bin/targets/x86/64/immortalwrt_x86-64_${kernel_version}.${patch_version}_bios.img.gz
else
    echo "Error: File immortalwrt-x86-64-generic-squashfs-combined.img.gz not found!"
    exit 1
fi

if [ -f "bin/targets/x86/64/immortalwrt-x86-64-generic-squashfs-combined-efi.img.gz" ]; then
    mv bin/targets/x86/64/immortalwrt-x86-64-generic-squashfs-combined-efi.img.gz \
       bin/targets/x86/64/immortalwrt_x86-64_${kernel_version}.${patch_version}_uefi.img.gz
else
    echo "Error: File immortalwrt-x86-64-generic-squashfs-combined-efi.img.gz not found!"
    exit 1
fi

# 直接指定文件名
openwrt_dev="immortalwrt_x86-64_${kernel_version}.${patch_version}_bios.img.gz"
openwrt_dev_uefi="immortalwrt_x86-64_${kernel_version}.${patch_version}_uefi.img.gz"
